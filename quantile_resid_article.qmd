...
The standard residual plot for both deviance residuals is predictably a bit weird and hard to interpret. The plot using Pearson residuals is not any more diagnostic. 
...
plot(fitted(rightModel), residuals(rightModel, type = "deviance"), 
     main = "correct model", xlab = "fitted values", ylab = "deviance residuals", 
...
Assessing if a generalized linear mixed model (GLMM) is appropriately specified for a given data set and data generating process is challenging since often GLMMs have no defined distribution for model residuals. Randomized quantile residuals are a useful model diagnostic tool for those situations. The implementation provided by the R package DHARMa is reliable and has compatability with a a broad selection of GLMM modelling packages. The R package 'performance' [@Ludecke2021] serves as a wrapper for DHARMa, implementing DHARMa function by default for generalized linear models. There is no existing implementation in SAS 9.4, but it is possible to manual write functions calculating quantile residuals.  
