...
The qq-plot on the left is comparing observed residual "quantiles" (these are actually probabilies) with expected quantiles that were generated assuming a uniform distribution (0, 1) simulated with `punif()`. The "KS Test" is a Kolmogorov-Smirnov test, using the function `ks.test()`.   
...
> *--F. Hartig, DHARMA Manual* from DHARMa::simulateResiduals()

### Conditional and Marginal Models

- Only fully supported for **lme4**, partially supported for **glmmTMB** (see [GitHub issue]() for more details)
