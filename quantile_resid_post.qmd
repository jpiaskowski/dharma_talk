...
Recall the correct model form is using a Poisson procesds:
...
A handy test evaluating how the number of zero's occurring measure up to the number of zero's expected for a given distribution and set of parameters. Like the other tets, the number of zero's under the simulations are compared to the observed zeros and an empirical probability of having more or less zero's than expected is reported.^[Is there another section in this guide b]
...
#### Example 1 
*Data set from the lme4 package*
...
This is a bit more difficult to implement and currently [not well supported](https://github.com/glmmTMB/glmmTMB/issues/888). The simulation setting overall for an object must be manually set to only use fixed effects. 
...
These would be need to be reset to random to return to conditional simulations:
...
```{r}
#| echo: true
r2a <- simulateResiduals(g2, plot=TRUE, n = 500, re.form = NULL)
```
...
This is equivalent to
 ...
 #| echo: true
#| eval: false
simulateResiduals(g2, plot=TRUE, n = 500, re.form = ~(1|herd))
```
...
```{r}
#| echo: true
r2b <- simulateResiduals(g2, plot=TRUE, n = 500)
```
...
#| echo: true
#| eval: false
simulateResiduals(g2, plot=TRUE, n = 500, re.form = ~0)
simulateResiduals(g2, plot=TRUE, n = 500, re.form = NA)
```


## Limitations to Randomized Quantile Residuals

There is a relative paucity of peer-reviewed literature. As described, Dunn and Smyth [-@dunn] established the basic framework for calculating randomized quantile residuals in a short communication, which Warton et al and [-@warton] expanded on and determined that this approach works for sparse (zero enriched), overdispersed and high dimension data (i.e. many independent variables). Warton established that this residual approach only required information on the marginal distribution of data and did not require understanding the joint distributions of data from marginal models nor did it need information on correlation structures in clustered data. Feng et al [-@feng] further developed a stronger theoretical foundation for randomized quantile residuals and evaluated the ability of these to detect deviations from non-linearity, overdispersion, and zero-inflation. The residual patterns were such that deviations from linearity for discrete and continous outcome variables, overdiserspion and model misspecification with regard to zero inflation was detectable. However, there has been no published work exploring the performance of randomized quantile residuals in a mixed model context (general or generalized). 

Randomized quantile residuals do not work with any categorical outcome. However, Gerber and Craig [-@gerber] have developed diagnostic residuals for categorical outcomes variables and implemented this approach in the R package '[MMLN](https://github.com/eaegerber/MMLN)'. 
...
- **Interpret DHARMa results as suggestions, not rules.** Although the hypothesis tests conducted by DHARMa provide an easy decision making tool, recall that no hypothesis provides an unambiguous answer. Outlier test results in particular should be treated with caution and not strictly diagnostic for individual observations.
