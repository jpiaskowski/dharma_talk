...
Below are examples of conditional and unconditional residual simulations. This example, used earlier, is from the ***lme4*** package. 
...
```{r}
r2a <- simulateResiduals(m2, plot=TRUE)
```

Unconditional Simulations

```{r}
r2b <- simulateResiduals(m2, plot=TRUE)
```
